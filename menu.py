...
def unlock_self_folder ():
    ...
    "  try { "
    "    $procs = Get-Process | Where-Object { $_.Path -like \"$target*\" -and $_.Id -ne $currentPid }; "
    "    foreach($p in $procs){ "
    "      try { "
    ...
    "  } "
    "  try { "
    "    $active = Get-Process | Where-Object { $_.Path -like \"$target*\" -and $_.Id -ne $currentPid -and $_.Id -ne $pPid }; "
    "    if(!$active){ "
    "      break; "
    "    } "
    "  } catch { "
