...
class UpdateChecker(QThread):
    update_checked = Signal(bool, object)
    def __init__(self, force_test=False):
        super().__init__()
        self.force_test = force_test
    def run(self):
        try:
            r = urllib.request.urlopen(GITHUB_RAW_URL, timeout=3)
            ...
            if latest:
                ...
                if local_tuple >= latest_tuple:
                    self.update_checked.emit(True, latest)
                else:
                    self.update_checked.emit(False, latest)
            else:
                self.update_checked.emit(True, None)
        except Exception:
            self.update_checked.emit(True, None)
class MainWindow(QMainWindow):
    ...
    def _on_update_checked(self, ok, latest):
        try:
