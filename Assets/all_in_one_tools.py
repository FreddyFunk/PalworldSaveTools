...
from ui_theme import apply_theme ,BG as DARK_BG ,GLASS as GLASS_BG ,TEXT as TEXT_COLOR ,ACCENT as ACCENT_COLOR ,BORDER as BORDER_COLOR ,BUTTON_BG ,BUTTON_HOVER ,MUTED as MUTED_COLOR ,EMPHASIS as EMPHASIS_COLOR ,ALERT as ALERT_COLOR ,SUCCESS as SUCCESS_COLOR ,ERROR as ERROR_COLOR ,BUTTON_FG ,BUTTON_PRIMARY ,BUTTON_SECONDARY ,FONT ,FONT_BOLD ,FONT_LARGE ,FONT_SMALL ,SPACE_SMALL ,SPACE_MEDIUM ,SPACE_LARGE ,CTK_BUTTON_CORNER_RADIUS ,CTK_FRAME_CORNER_RADIUS ,TREE_ROW_HEIGHT 
apply_theme ()
    ...
    def monitor (t0_local ):
        for future in as_completed (futures ):
            try :
                ...
                print (f'DPS processing failed: {e }')
...
def start_dps_processing ():
    ...
    dps_futures =[dps_executor .submit (process_dps_save ,uid ,pname ,dps_file ,log_folder )for uid ,pname ,dps_file ,log_folder in dps_tasks ]
    return dps_futures 
    ...
    for itm in cmap :
        try :
            ...
            if 'IsPlayer'in raw_p :
                ...
                if uid :
                    owner_nicknames [str (uid ).lower ()]=nn 
        except :
            pass 
...
def get_players ():
    ...
    for g in wsd ['GroupSaveDataMap']['value']:
        ...
        for p in players :
            ...
            lastseen ='Unknown'if last is None else format_duration ((tick -last )/10000000.0 )
...
def delete_base_camp (base ,guild_id ,loaded_json ):
    ...
    if guild_id and (not are_equal_uuids (base_group_id ,guild_id )):
        return False 
    ...
    if group_data :
        ...
        if base_id in base_ids :
            ...
            base_ids .pop (idx )
            if mp_points and idx <len (mp_points ):
                mp_points .pop (idx )
    map_objs =wsd ['MapObjectSaveData']['value']['values']
    map_obj_ids_to_delete ={m .get ('Model',{}).get ('value',{}).get ('RawData',{}).get ('value',{}).get ('instance_id')for m in map_objs if m .get ('Model',{}).get ('value',{}).get ('RawData',{}).get ('value',{}).get ('base_camp_id_belong_to')==base_id }
...
def delete_selected_player ():
    ...
    if deleted :
        char_map =wsd .get ('CharacterSaveParameterMap',{}).get ('value',[])
        char_map [:]=[entry for entry in char_map if str (entry .get ('key',{}).get ('PlayerUId',{}).get ('value','')).replace ('-','')!=uid and str (entry .get ('value',{}).get ('RawData',{}).get ('value',{}).get ('object',{}).get ('SaveParameter',{}).get ('value',{}).get ('OwnerPlayerUId',{}).get ('value','')).replace ('-','')!=uid ]
        refresh_all ()
        refresh_stats ('After')
...
def delete_inactive_bases ():
    ...
    for g in wsd ['GroupSaveDataMap']['value']:
        ...
        for p in g ['value']['RawData']['value'].get ('players',[]):
            ...
            if last_online is None or (tick -last_online )/10000000.0 /86400 <d :
                allold =False 
                break 
        if allold :
            to_clear .append (gid )
...
def delete_orphaned_bases ():
    folder =current_save_path 
    if not folder :
        return print (t ('guild.rebuild.no_save'))
    wsd =loaded_level_json ['properties']['worldSaveData']['value']
    ...
    refresh_stats ('After')
    if cnt >0 :
        print (t ('bases.orphaned_deleted',count =cnt ))
...
def delete_unreferenced_data ():
    ...
    result_msg =t ('cleanup.summary',removed_players =len (all_removed_uids ),unref =len (unreferenced_uids ),invalid =len (invalid_uids ),pals =removed_pals ,guilds =removed_guilds ,broken =removed_broken ,drops =removed_drops ,mapobjects =mapobject_count )
    print (result_msg )
...
def delete_inactive_players (folder_path ,inactive_days =30 ):
    ...
    if to_delete_uids :
        ...
        result_msg =t ('cleanup.preview_summary',before =total_players_before ,marked =len (deleted_info ),after =total_players_after ,pals =removed_pals )
        print (result_msg )
...
def on_guild_members_search (q =None ):
    ...
    player_lookup_map ={uid .replace ('-',''):(uid ,name ,gid ,seen ,level )for uid ,name ,gid ,seen ,level in get_players ()}
    if q is None :
...
def pil_text_to_surface (text ,size =20 ,color =(255 ,255 ,255 )):
    font_paths =['C:\\Windows\\Fonts\\msgothic.ttc','C:\\Windows\\Fonts\\YuGothicUI.ttf','C:\\Windows\\Fonts\\YuGothic.ttf','C:\\Windows\\Fonts\\meiryo.ttc','C:\\Windows\\Fonts\\meiryob.ttc','C:\\Windows\\Fonts\\msmincho.ttc']
    font =None 
...
def show_base_map ():
    ...
    def get_last_seen (g_val ,current_tick ):
        ...
        diff =(current_tick -max (times ))/10000000.0 
    ...
    def truncate_text (text ,max_w ,use_font ):
        ...
        if use_font .size (text )[0 ]<=max_w :
            return text 
        while use_font .size (text +'...')[0 ]>max_w and len (text )>1 :
            text =text [:-1 ]
        return text +'...'
    ...
    def draw_sidebar_ui (win_w ,win_h ):
        ...
        if active_input :
            pygame .draw .rect (sidebar_surface ,(0 ,150 ,255 ),input_rect ,width =2 ,border_radius =6 )
        ...
        for lbl ,off in cols :
            sidebar_surface .blit (small_font .render (lbl ,True ,(160 ,160 ,160 )),(off ,y_h +7 ))
        ...
        for gid ,guild in filtered_guilds .items ():
            ...
            if gid in expanded_guilds :
                for b in guild ['bases']:
                    ...
                    if selected_item and selected_item [0 ]=='base'and (selected_item [1 ]==b ):
                        pygame .draw .rect (sidebar_surface ,(70 ,70 ,70 ),br ,border_radius =4 )
    ...
    def draw_list_item_to_surf (surf ,gid ,guild ,y ,is_selected ,is_hovered ):
        ...
        if is_selected :
            pygame .draw .rect (surf ,(60 ,80 ,100 ),rect ,border_radius =4 )
        elif is_hovered :
    ...
    while running :
        ...
        if is_animating :
            ...
            if abs (offset_x -target_offset_x )<0.1 and abs (zoom -target_zoom )<0.001 :
                is_animating =False 
        for ev in pygame .event .get ():
            if ev .type ==pygame .QUIT :
                ...
                if ev .button ==1 :
                    ...
                    last_click_time =now 
                    if context_menu_pos :
                        if context_menu_target [0 ]=='base':
                            opts =[t ('delete.base'),t ('button.export')]
                        else :
                            opts =[t ('guild.rename.title'),t ('delete.guild'),t ('button.import')]
                            ...
                            if 0 <=idx <len (opts ):
                                ...
                                if act ==t ('delete.base'):
                                    ...
                                    for item in base_tree .get_children ():
                                        if str (base_tree .item (item )['values'][0 ])==bid :
                                            base_tree .selection_set (item )
                                            delete_selected_base ()
                                            break 
                                    guilds_all =get_guild_bases_live ()
                                    filtered_guilds =filter_guilds_and_bases (guilds_all ,user_input )
                                    ...
                                    if not still_exists :
                                        ...
                                        trigger_blocked ((cx -offset_x )*zoom ,(cy -offset_y )*zoom ,bid )
                                    refresh_all ()
                                elif act ==t ('delete.guild'):
                                    gid =str (context_menu_target [1 ])
                                    for item in guild_tree .get_children ():
                                        if str (guild_tree .item (item )['values'][1 ])==gid :
                                            guild_tree .selection_set (item )
                                            delete_selected_guild ()
                                            break 
                                    guilds_all =get_guild_bases_live ()
                                    filtered_guilds =filter_guilds_and_bases (guilds_all ,user_input )
                    ...
                    elif pygame .Rect (w -sidebar_w +15 ,12 ,sidebar_w -30 ,32 ).collidepoint (ev .pos ):
                        active_input =True 
                    else :
                    ...
                    if not found and ev .pos [0 ]>map_w :
                        ...
                        for gid ,guild in filtered_guilds .items ():
                            ...
                            if found :
                                break 
                    if found :
                        context_menu_pos ,context_menu_target =(ev .pos ,found )
                ...
                if mouse_pos [0 ]>map_w :
                    scroll_offset =max (0 ,scroll_offset -ev .y *40 )
                else :
                ...
                if zoom <=absolute_min_zoom :
                    zoom =absolute_min_zoom 
                    target_zoom ,target_offset_x ,target_offset_y ,_ =get_initial_state (ev .w -sidebar_w ,ev .h )
                    is_animating =True 
                ...
                if 0 <=px <map_w and 0 <=py <h :
                    ...
                    for eff in active_blocked_effects :
                        if eff ['target_id']==str (b ['base_id']):
                            if t_now -eff ['start_time']<0.8 :
                                b_shx ,b_shy =(random .randint (-5 ,5 ),random .randint (-5 ,5 ))
        ...
        for shock in active_shocks [:]:
            ...
            if el >1.0 :
                active_shocks .remove (shock )
                continue 
            shock ['radius']+=20 
            shock ['alpha']=int (255 *(1 -el ))
        ...
        for eff in active_blocked_effects [:]:
            ...
            if el >0.8 :
                active_blocked_effects .remove (eff )
                continue 
        ...
        if hb and (not context_menu_pos ):
            draw_tooltip ([f"Guild: {hb ['guild_name']}",f"Leader: {hb ['leader_name']}",f"ID: {hb ['base_id']}",f"Coords: X:{int (hb ['coords'][0 ])} Y:{int (hb ['coords'][1 ])}"],mouse_pos ,map_w )
        if mouse_pos [0 ]<map_w :
        ...
        if context_menu_pos :
            if context_menu_target [0 ]=='base':
                ...
                opts =[t ('guild.rename.title'),t ('delete.guild'),t ('button.import')]
            cmr =pygame .Rect (context_menu_pos [0 ],context_menu_pos [1 ],160 ,len (opts )*32 )
            pygame .draw .rect (screen ,(40 ,40 ,40 ),cmr ,border_radius =4 )
            pygame .draw .rect (screen ,(80 ,80 ,80 ),cmr ,1 ,border_radius =4 )
...
class KillNearestBaseDialog (tk .Toplevel ):
    def __init__ (self ,master =None ):
        ...
        style .map ('Dark.TButton',background =[('active',BUTTON_HOVER ),('!disabled',BUTTON_FG )],foreground =[('disabled','#888888'),('!disabled',TEXT_COLOR )])
        style .configure ('TRadiobutton',background =GLASS_BG ,foreground =TEXT_COLOR ,font =font_style )
        style .map ('TRadiobutton',background =[('active','#3a3a3a'),('!active',GLASS_BG )],foreground =[('active',TEXT_COLOR ),('!active',TEXT_COLOR )])
        self .setup_ui ()
    ...
    def on_generate (self ):
        ...
        try :
            ...
            if ftype =='3'and (inactivity_days is None or max_level is None ):
                ...
                return 
            result =self .parse_log (inactivity_days =inactivity_days if ftype in ('1','3')else None ,max_level =max_level if ftype in ('2','3')else None )
            if not result :
    ...
    def parse_log (self ,inactivity_days =None ,max_level =None ):
        ...
        for guild in guilds :
            ...
            if not players_data or not bases :
                continue 
            guild_name =re .search ('Guild: (.+?) \\|',guild )
            guild_leader =re .search ('Guild Leader: (.+?) \\|',guild )
            guild_id =re .search ('Guild ID: ([a-f0-9-]+)',guild )
            ...
            if inactivity_days is not None :
                if any (('d'not in player [7 ]or int (re .search ('(\\d+)d',player [7 ]).group (1 ))<inactivity_days for player in players_data )):
                    continue 
            ...
            for _ ,raw_data in filtered_bases :
                coords =re .findall ('[-+]?\\d*\\.\\d+|[-+]?\\d+',raw_data )
                if len (coords )>=3 :
...
def copy_stats_to_clipboard (stat_labels ):
    ...
    copy_content +=header_template .format (type =t ('deletion.stats.field'),before =t ('deletion.stats.before'),after =t ('deletion.stats.after'),result =t ('deletion.stats.result'))+'\n'
    for field in fields :
    ...
    try :
        ...
        if w_cont :
            ...
            for slot in w_cont ['value']['Slots']['value'].get ('values',[]):
                ...
                if char_entry :
                    export_data ['characters'].append (copy .deepcopy (char_entry ))
    except :
        pass 
    for obj in map_objs :
        mr =_get_model_raw (obj )
        ...
        try :
            ...
            for mod in mm :
                ...
                if 'ItemContainer'in str (mod .get ('key','')):
                    ...
                    if ic :
                        ...
                        for slot in item_slots :
                            ...
                            if loc_id !='00000000-0000-0000-0000-000000000000':
                                ...
                                if d_entry :
                                    export_data ['dynamic_items'].append (copy .deepcopy (d_entry ))
                            cleaned_slots .append (slot )
                        nic ['value']['Slots']['value']['values']=cleaned_slots 
                    ...
                    if cc :
                        export_data ['char_containers'].append (copy .deepcopy (cc ))
        except :
            pass 
...
def all_in_one_tools ():
    ...
    try :
        ...
        pass 
    window .refresh_elements ={'menu_buttons':[],'main_labels':[],'version_labels':[],'treeview_headings':[],'notebook_tabs':[],'menus':[],'search_labels':[],'stat_key_labels':[]}
    ...
    s .map ('Vertical.TScrollbar',background =[('active','#444444'),('!active',GLASS_BG )],troughcolor =[('active',GLASS_BG ),('!active',GLASS_BG )],arrowcolor =[('active',TEXT_COLOR ),('!active',TEXT_COLOR )],bordercolor =[('active',GLASS_BG ),('!active',GLASS_BG )])
    MENU_BG =GLASS_BG 
    MENU_FG =TEXT_COLOR 
    MENU_ACTIVE_BG =ACCENT_COLOR 
    ...
    def create_menu_button (parent ,text_key ,menu_object ):
        button =ctk .CTkButton (parent ,text =t (text_key ),fg_color =BUTTON_BG ,hover_color =BUTTON_HOVER ,width =50 ,height =18 ,corner_radius =0 ,font =menu_button_font )
    ...
    def save_exclusions_func ():
        with open ('deletion_exclusions.json','w')as f :
    ...
    if info :
        ...
        if not update_available :
            ...
            update_label =ctk .CTkLabel (version_frame ,text =f"{t ('update.latest')}: {latest }",text_color ='white',font =('Consolas',11 ,'bold','underline'),cursor ='hand2')
    ...
    status_label_frame =ctk .CTkFrame (info_container ,fg_color ='transparent')
    status_label_frame .grid (row =0 ,column =0 ,sticky ='nsew',padx =(0 ,SPACE_MEDIUM ))
    status_label_frame .columnconfigure (0 ,weight =1 )
    status_label_frame .rowconfigure (0 ,weight =1 )
    status_label_frame .rowconfigure (1 ,weight =1 )
    status_label_frame .rowconfigure (2 ,weight =1 )
    ...
    player_result .grid (row =0 ,column =0 ,sticky ='w',pady =(0 ,3 ))
    ...
    guild_result .grid (row =1 ,column =0 ,sticky ='w',pady =(0 ,3 ))
    ...
    notebook =ctk .CTkTabview (window ,width =1100 ,height =600 )
    notebook .grid (row =2 ,column =0 ,padx =10 ,pady =(0 ,10 ),sticky ='nsew')
    ...
    pframe ,player_tree ,player_search_entry ,player_search_label =create_search_panel (player_tab ,'deletion.search_players',player_search_var ,on_player_search ,('Name','Last','Level','Pals','UID','GName','GID'),(t ('deletion.col.player_name'),t ('deletion.col.last_seen'),t ('deletion.col.level'),t ('deletion.col.pals'),'UID',t ('deletion.col.guild_name'),t ('deletion.col.guild_id')),(140 ,120 ,60 ,60 ,150 ,180 ,180 ),900 ,600 ,tree_height =25 )
    ...
    for col ,key in zip (('Name','Last','Level','Pals','UID','GName','GID'),('deletion.col.player_name','deletion.col.last_seen','deletion.col.level','deletion.col.pals','UID','deletion.col.guild_name','deletion.col.guild_id')):
    ...
    gframe ,guild_tree ,guild_search_entry ,guild_search_label =create_search_panel (guild_tab ,'deletion.search_guilds',guild_search_var ,on_guild_search ,('Name','ID'),(t ('deletion.col.guild_name'),t ('deletion.col.guild_id')),(150 ,150 ),700 ,400 ,tree_height =15 )
    ...
    gm_frame ,guild_members_tree ,guild_members_search_entry ,guild_members_search_label =create_search_panel (guild_tab ,'deletion.guild_members',guild_members_search_var ,on_guild_members_search ,('Name','Last','Level','Pals','UID'),(t ('deletion.col.member'),t ('deletion.col.last_seen'),t ('deletion.col.level'),t ('deletion.col.pals'),'UID'),(200 ,120 ,60 ,100 ,300 ),800 ,200 ,tree_height =10 )
    ...
    bframe ,base_tree ,base_search_entry ,base_search_label =create_search_panel (base_tab ,'deletion.search_bases',base_search_var ,on_base_search ,('ID','GuildID','GuildName'),(t ('deletion.col.base_id'),t ('deletion.col.guild_id'),t ('deletion.col.guild_name')),(200 ,250 ,250 ),800 ,600 ,tree_height =25 )
