...
def file_picker(ext):
    ...
    path = None
    if ext == 'sav':
        path = filedialog.askopenfilename(title='Select WorldOption.json', filetypes=[('WorldOption.json', 'WorldOption.json')])
    elif ext == 'json':
        path = filedialog.askopenfilename(title='Select WorldOption.sav', filetypes=[('WorldOption.sav', 'WorldOption.sav')])
    root.destroy()
    return path
...
def convert_worldoption(ext):
    ...
    QMessageBox.information(parent, t('tool.convert.done'), t('tool.convert.worldoption_done', source=worldoption_file, target=output_path))
    return True
def main():
