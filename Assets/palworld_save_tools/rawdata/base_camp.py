...
def decode_bytes(parent_reader: FArchiveReader, b_bytes: Sequence[int]) -> dict[str, Any]:
    ...
    data = {'id': reader.guid(), 'name': reader.fstring(), 'state': reader.byte(), 'transform': reader.ftransform(), 'area_range': reader.float(), 'group_id_belong_to': reader.guid(), 'fast_travel_local_transform': reader.ftransform(), 'owner_map_object_instance_id': reader.guid()}
