    ...
    for c in char_data:
        ...
        if struct_type == 'PalIndividualCharacterSaveParameter':
            ...
            if owner_uid and str(owner_uid) != '00000000-0000-0000-0000-000000000000':
                ...
                total_workers += 1
    return {'Total Players': str(total_players), 'Total Active Guilds': str(total_guilds), 'Total Bases': str(total_bases), 'Total Overall Pals': str(total_pals), 'Total Owned Pals': str(total_owned), 'Total Caught Pals': str(total_caught), 'Total Worker/Dropped Pals': str(total_workers)}
def get_guild_base_data_from_save():
    ...
    for entry in group_data:
        ...
        if leader_name == 'Unknown' and (not raw_data.get('players')):
            continue
        current = {'Guild': guild_name, 'Guild Leader': leader_name}
...
def render_text_pil(text, size=20, color=(255, 0, 0), font_path=None):
    font_paths = ['C:\\Windows\\Fonts\\msgothic.ttc', 'C:\\Windows\\Fonts\\YuGothicUI.ttf', 'C:\\Windows\\Fonts\\YuGothic.ttf', 'C:\\Windows\\Fonts\\meiryo.ttc']
    if font_path:
...
def create_world_map():
    ...
    if os.path.exists(logo_path):
        try:
            ...
            logo_width = int(overlay.width * 0.2)
...
def generate_map(parent=None):
    if not constants.loaded_level_json:
        _show_message_threadsafe(t('error.title') if t else 'Error', t('guild.rebuild.no_save') if t else 'No save loaded', 'error')
        return None
    ...
    try:
        ...
        if os.path.exists(map_path):
            ...
            _show_message_threadsafe(t('success.title') if t else 'Success', t('mapgen.opening') if t else 'Map generated successfully!', 'info')
        else:
            _show_message_threadsafe(t('error.title') if t else 'Error', t('mapgen.not_found') if t else 'updated_worldmap.png not found.', 'error')
        ...
        _show_message_threadsafe(t('error.title') if t else 'Error', error_msg, 'error')
        print(error_msg)
        import traceback
        traceback.print_exc()
        return None